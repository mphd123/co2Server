...
public class SensorDataDB implements AutoCloseable {
    ...
    public SensorDataDB(@Value("${co2.db.path}") String connectionString) throws Exception {
        try {
    ...
    public void addEntry(int co2, int temp, String sensorName, Timestamp date) throws Exception {
        try (PreparedStatement p = connection.prepareStatement(Co2Table.INSERT)) {
            ...
            p.setTimestamp(Co2Table.INSERT_Date_INDEX,date);
